...
async def test_routed_runtime_reconnect_uses_primary_transport_replacement(
    ...
    # Isolate transport-owner resolution from the separate common-wiring
    # regression above. Production attaches this at the same shared seam.
    ...
    cast(Any, routed_adapter).gateway_runner = runner
    ...
    # Production topic recovery and attribution middleware use
    # dataclasses.replace(). Transport provenance must survive those rewrites.
...
@pytest.mark.asyncio
async def test_reconnect_routes_normal_text_and_images_to_replacement(
    monkeypatch,
    tmp_path,
):
    stale_adapter = CaptureSlackAdapter()
    runner = _wire_runner(
        ...
        tmp_path,
        stale_adapter,
        [_successful_result("normal response\n![plot](https://example.com/plot.png)")],
    )
    ...
    stale_adapter.send_multiple_images = AsyncMock(
        side_effect=AssertionError("stale transport must not send attachments")
    )
    replacement.send_multiple_images = AsyncMock(return_value=[])
...
def test_missing_secondary_transport_owner_keeps_secondary_policy_scope():
    ...
    assert runner._adapter_profile_for_source(source) == "coder"
    assert stale_adapter._final_delivery_adapter(source) is None


@pytest.mark.asyncio
async def test_missing_transport_owner_does_not_send_backend_notice(
    monkeypatch,
    tmp_path,
    caplog,
):
    ...
    source = stale_adapter.build_source(
        ...
        thread_id="171717",
        user_id="U123",
        message_id="m-missing-owner-notice",
    )
    event = MessageEvent(
        text="hello",
        source=source,
    ...
    assert stale_adapter.processing_hooks[-1] == (
        "complete",
        "m-missing-owner-notice",
        ProcessingOutcome.FAILURE,
    )
    assert "response_delivery_dropped" in caplog.text


@pytest.mark.asyncio
async def test_missing_transport_owner_does_not_send_text_or_attachments(
    monkeypatch,
    tmp_path,
    caplog,
):
    ...
    stale_adapter.play_tts = AsyncMock(
        side_effect=AssertionError("stale transport must not send TTS")
    )
    stale_adapter.send_multiple_images = AsyncMock(
        side_effect=AssertionError("stale transport must not send attachments")
    )
    ...
    ledger_record = MagicMock(
        side_effect=AssertionError("missing owner must not create a ledger obligation")
    )
    monkeypatch.setattr(delivery_ledger, "record_obligation", ledger_record)
    source = stale_adapter.build_source(
        chat_id="C123",
        chat_type="channel",
        thread_id="171717",
        user_id="U123",
        message_id="m-missing-owner-normal",
    )
    event = MessageEvent(
        text="hello",
        source=source,
...
@pytest.mark.asyncio
async def test_replacement_delivery_exception_never_falls_back_to_stale_adapter(
    monkeypatch,
    tmp_path,
):
    stale_adapter = CaptureSlackAdapter()
    ...
    runner._share_backend_notice_state(replacement, profile_name="default")
    runner.adapters[Platform.SLACK] = replacement
    replacement._send_with_retry = AsyncMock(
    ...
    assert "hooks.slack.com" not in replacement.sent[0]["content"]
    assert "super-secret-token" not in replacement.sent[0]["content"]


def test_restored_named_profile_source_uses_runtime_profile_adapter():
    runner = cast(Any, object.__new__(gateway_run.GatewayRunner))
    primary = CaptureSlackAdapter()
    coder = CaptureSlackAdapter()
    runner.adapters = {Platform.SLACK: primary}
    runner._profile_adapters = {"coder": {Platform.SLACK: coder}}
    ...
    assert source._transport_profile is None
    assert runner._adapter_for_source(source) is coder
    assert runner._adapter_profile_for_source(source) == "coder"


def test_restored_default_source_uses_primary_adapter():
    runner = cast(Any, object.__new__(gateway_run.GatewayRunner))
    primary = CaptureSlackAdapter()
    runner.adapters = {Platform.SLACK: primary}
    runner._profile_adapters = {}
    source = SessionSource.from_dict(
        SessionSource(platform=Platform.SLACK, chat_id="C123").to_dict()
    )
