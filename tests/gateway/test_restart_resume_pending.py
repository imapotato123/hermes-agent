...
async def test_startup_auto_resume_uses_durable_owner_not_routed_runtime():
    ...
    pending_entry.transport_owner_stamped = True
    pending_entry.transport_platform = Platform.TELEGRAM.value
    pending_entry.transport_profile = "alpha"
    runner.session_store._entries = {pending_entry.session_key: pending_entry}
...
@pytest.mark.asyncio
async def test_startup_auto_resume_leaves_ownerless_legacy_entry_pending():
    """Legacy routing metadata cannot nominate the routed runtime's bot."""
    from tests.gateway.restart_test_helpers import RestartTestAdapter
    ...
    source = SessionSource.from_dict(
        make_restart_source(chat_id="legacy-ownerless").to_dict()
    )
    source.profile = "beta"
    pending_entry = SessionEntry(
